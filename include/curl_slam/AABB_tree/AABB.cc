...
Tree::Tree(unsigned int dimension_, double skinThickness_, unsigned int nParticles, bool touchIsOverlap_)
...
Tree::Tree(unsigned int dimension_, double skinThickness_, const std::vector<bool> &periodicity_,
           const std::vector<double> &boxSize_, unsigned int nParticles, bool touchIsOverlap_)
...
void Tree::setPeriodicity(const std::vector<bool> &periodicity_) { periodicity = periodicity_; }

void Tree::setBoxSize(const std::vector<double> &boxSize_) { boxSize = boxSize_; }
...
void Tree::insertParticle(unsigned int particle, double fattenNum, std::vector<double> &lowerBound,
    ...
    // Fatten the AABB.
    for (unsigned int i = 0; i < dimension; i++) {
        nodes[node].aabb.lowerBound[i] -= (skinThickness * size[i] + fattenNum);
        nodes[node].aabb.upperBound[i] += (skinThickness * size[i] + fattenNum);
    }
    nodes[node].aabb.surfaceArea = nodes[node].aabb.computeSurfaceArea();
    nodes[node].aabb.centre = nodes[node].aabb.computeCentre();
...
void Tree::insertGroundParticle(unsigned int particle, double fattenNum, double fattenScale,
    ...
    //        nodes[node].aabb.upperBound[i] += (skinThickness * size[i] + fattenNum);
    //    }
    nodes[node].aabb.lowerBound[2] -= (skinThickness * size[2] + fattenScale * fattenNum);
    nodes[node].aabb.upperBound[2] += (skinThickness * size[2] + fattenScale * fattenNum);
    nodes[node].aabb.surfaceArea = nodes[node].aabb.computeSurfaceArea();
    nodes[node].aabb.centre = nodes[node].aabb.computeCentre();
...
bool Tree::updateParticle(unsigned int particle, std::vector<double> &position, double radius, bool alwaysReinsert) {
    // Validate the dimensionality of the position vector.
    if (position.size() != dimension) {
        throw std::invalid_argument("[ERROR]: Dimensionality mismatch!");
    }
...
const AABB &Tree::getAABB(unsigned int particle) { return nodes[particleMap[particle]].aabb; }

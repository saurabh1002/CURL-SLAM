...
// using SCPointType = pcl::PointXYZI; // using xyz only. but a user can exchange the original bin encoding function (i.e.,
                                    // max hegiht) to max intensity (for detail, refer 20 ICRA Intensity Scan Context)
using SCPointType = PointXYZITL;
...
class SCManager {
  public:
    ...
    std::pair<int, float> detectLoopClosureID(const int query_idx); // query by index
  ...
  public:
    ...
    const double SC_DIST_THRES =
        0.13; // empirically 0.1-0.2 is fine (rare false-alarms) for 20x60 polar context (but for 0.15 <, DCS or ICP fit
              // score check (e.g., in LeGO-LOAM) should be required for robustness)

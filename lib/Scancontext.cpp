...
std::pair<int, float> SCManager::detectLoopClosureID(const int query_idx) {
    int loop_id = -1;
    if (query_idx < 0 || query_idx >= static_cast<int>(polarcontexts_.size())) {
        return {loop_id, 0.0f};
    }
    if (polarcontexts_.size() < static_cast<std::size_t>(NUM_EXCLUDE_RECENT + 1)) {
        return {loop_id, 0.0f};
    }

    MatrixXd curr_desc = polarcontexts_[query_idx];
    double min_dist = 10000000;
    int nn_align = 0;
    int nn_idx = -1;

    for (int candidate_idx = 0; candidate_idx < static_cast<int>(polarcontexts_.size()); ++candidate_idx) {
        if (candidate_idx == query_idx) {
            continue;
        }
        if (std::abs(candidate_idx - query_idx) < NUM_EXCLUDE_RECENT) {
            continue;
        }
